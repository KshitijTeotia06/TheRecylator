...
//  KshitijTeotia_RecyclingApplication
//
//  Created by SKumar on 7/18/20.
//  Copyright © 2020 Kshitij. All rights reserved.
//
...
class ViewController: UIViewController, UINavigationControllerDelegate {
    ...
    lazy var detectionRequest: VNCoreMLRequest = {
        do {
            let model = try VNCoreMLModel(for: trashClassifer_2().model)
    ...
    func drawDetectionsOnPreview(detections: [VNRecognizedObjectObservation]) {
        ...
        for detection in detections {
            ...
            let a = detection.labels.map({"\($0.identifier) confidence: \($0.confidence)"}).joined(separator: "\n")
            print("--------asdf-----")
            print(detection.labels[0].identifier)
            answerLabel.text = detection.labels[0].identifier
            print("------------")
